  ...
  try {
    (await psqlDb).connect(async (connection) => {
      data = await connection.query(
        sql<queries.User>`SELECT * FROM users WHERE email_id=${email}`
      );
      return data;
    });
  ...
  try {
    (await psqlDb).connect(async (connection) => {
      const columnData = await connection.query(
        sql<queries.Column>`SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'users';`
      );
  ...
  try {
    (await psqlDb).connect(async (connection) => {
      const data = await connection.query(
        sql<queries.User>`SELECT * FROM users WHERE email_id=${email_id}`
      );
      user = data;
...
async function getUsers(req: Request, res: Response) {
  try {
    (await psqlDb).connect(async (Connection) => {
      const data = await Connection.query(sql<queries.User>`SELECT * FROM users`);
      if (data.rowCount > 0) {
